...
On top of that sit two layers of *permission memory*: an **allowlist** (allowed directly from then on, with no model call) and a **denylist** (handed to you directly, with no model call), plus **automatic promotion** — after the same permission in the same project has been approved manually a few times in a row, the plugin promotes it to a review-free rule. Both lists exist at **project** and **global** scope, and a rule can come either from you promoting/demoting a record in the timeline or from a rule suggestion the Reviewer model produced during a review.
