...
`dsh-auto-pass` 为 DeepSeek Harness 的 WebUI 增加 `自动审批` 权限档位。每个需要审批的动作都会先交给一个全新且受限的 DSH 子 Agent 审查，插件只自动放行审查通过（allow）的请求；模型拒绝、宿主安全降级和审查失败一律交回 DSH 原生人工审批链，由用户决定——插件绝不代替用户拒绝。
...
- 每行显示时间、工具名、结论（`自动批准` / `转人工` / `审查未完成`）、审批意见摘要，以及命中名单时的白/黑名单标记；转人工的还会显示你最终的选择；展开可见风险等级、用户授权、理由、审批原因、动作参数（裁剪到 500 字符）、耗时、Reviewer 会话与调查步数。
