...
- 通知通过 `agent.inject({ source: { kind: 'plugin', plugin: 'dsh-auto-pass', form: 'notice' } })` 写进父 session；allow 与转人工用不同 summary。**正文只有一行**（`injectReviewNotice` + `NOTICE_LABELS`）：`Auto Approve 已自动批准 bash · low/high · 2 步 · 理由：…`，上限 `MAX_NOTICE_LINE_CHARS = 240`；Reviewer 会话、建议规则、命中规则全文都不进上下文（留在审批时间线与宿主日志）。原因见下一条：这行通知是模型能看到的，必须足够短。
...
- **时间线行内展示**：折叠态除时间/工具/结论外，还有三行内容——**审批意见**（`record.rationale`，CSS 单行截断）、**决策来源** chip、**命中名单** chip；后两者共用 `.ap-hitRow` / `.ap-hitText` 同一形态（chip + 说明文字）。
- **命中 chip 带规则来源**：`describeHit` 把 `hit.rule.source` 写进记录的 `policy.source`（`user`=你手动加的，`model`=模型建议，`memory`=连续放行升级），时间线渲染成 `白名单·自动` / `黑名单·手动`（`user` → 手动，其余 → 自动）。**老记录没有该字段**，客户端于是按 `record.policy.ruleId` 去当前策略快照里反查规则的 `source`（`ruleSourceOf()`）——所以时间线挂载时必须也拉一次 `/policy`（`void policyStore.load()`），否则反查不到。`decidedBy` 同样只写在新记录上，客户端对老记录按 `verdict`/`outcome` 兜底推断。
- **决策来源标签（自动 / 人工）**：记录字段 `decidedBy = 'auto' | 'human'`，由宿主 `decisionSource(settled, decision)` 写入——插件自己自动放行的记 `auto`，转交人工后由人决定（通过/拒绝/无人应答）的记 `human`；客户端渲染成 `[自动] 模型审查通过，插件自动放行` / `[人工] 人工审批 · 已批准|已拒绝|无人应答|已取消`，老记录缺该字段时按 `verdict`/`outcome` 兜底推断。**它必须像「命中名单」那样单独一行**，不要塞进结论徽标前面（用户明确指出过）。注入通知的一行摘要则用 `[自动]` / `[人工]` 前缀（纯文本没有 chip 形态）。
