...
- **时间线行内展示**：折叠态除时间/工具/结论外，还有三行内容——**审批意见**（`record.rationale`，CSS 单行截断）、**决策来源** chip、**命中名单** chip（`record.policy.list` → 白名单/黑名单 + 作用域 + 规则标签）；后两者共用 `.ap-hitRow` / `.ap-hitText` 同一形态（chip + 说明文字）。
