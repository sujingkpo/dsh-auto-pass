...
- **命中 chip 带规则来源**：`describeHit` 已把 `hit.rule.source` 写进记录的 `policy.source`（`user`=你手动加的，`model`=模型建议，`memory`=连续放行升级），时间线渲染成 `白名单·自动` / `黑名单·手动`（`user` → 手动，其余 → 自动；老记录没有该字段时只显示 `白名单`，不加后缀）。
