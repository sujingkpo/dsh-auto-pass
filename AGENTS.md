...
- **权限档位的图标是 DSH 客户端硬编码的，第 4 个档位拿不到（2026-09-15 查证）**：输入框上方档位 chip/菜单里的图标来自 `@deepseek-ai/dsh-client-ui-conversation` 的 `permissionGlyphs` —— 一个只含 `read-only` / `workspace-write` / `danger-full-access` 三个 **id** 的 `Map`（注释原话：host-configured names outside the design set get none）；设置页的权限菜单（`dsh-client-ui-permission-presets`）更只有纯文字 `{id,label}`。宿主 `presets.<id>` 的 schema 只有 `{sandbox, approval, name, description}`，**没有图标位**（`name` 里的字符是唯一的可视抓手，彩色 emoji 又太扎眼），所以现在由插件自己画：`src/client.js` 的 `installPresetIcon()` 用 MutationObserver（**childList + characterData**：切换档位时 label 文本是在同一个 span 上改写的，只观察节点增删永远补不上图标）在档位 chip 与下拉菜单项里找文字精确等于档位名的**纯文字** `<span>`（`isPresetLabelNode` 只认 textContent + 无子元素，不依赖 DSH 的哈希类名），打上 `ap-presetGlyph`（菜单项再加 `ap-presetGlyphMenu`）；选择器是 `[role=menu] span, [role=menuitem] span, [aria-haspopup=menu] span, button span, [aria-label*=档位名] span` —— **composer 的 trigger 没有 `aria-haspopup`**（菜单原语只渲染 anchor、不补属性），只按 aria 属性选会漏掉 chip（第一版就是这么漏的，表现为「选完档位 chip 没图标」）；反正按文字精确过滤，选择器放宽不会误伤，同一帧内多次变动用 rAF 合并。CSS 用 `::before` + `mask` 画内联「盾牌 + A」SVG：**chip 14px / currentColor** 对齐内置 `.triggerIcon svg`，**菜单项 16px / `--dsw-alias-label-tertiary` / 8px 间距** 对齐内置 `.itemIcon`（第一版统一 14px + currentColor，被用户指出「图标太小」）。形状是先渲成 PNG 比对过 A/B/C 三版笔画粗细才定的 B 版；`name` 因此保持纯中文 `自动审批`（非 kebab-case 的 name 会被两处客户端原样透传）。**不要为了图标去改 id**：`src/index.js` 的闸门是 `selectedPermissionPreset(session) !== 'auto-approve'`，且三个内置 id 各有语义不可顶替。
